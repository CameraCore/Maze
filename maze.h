...
class WeightedPoint
{
	public:
		//Holds a point
		point _point;
		//The cost to this point from the parent point
		int _cost;
		//The total distance traveled to get to this point
		int _tentative;
};
struct CostCompare
{
	bool operator()(const WeightedPoint& firstPoint, const WeightedPoint& secondPoint) const
	{
		//Compare point one's tentative distance to point two's. Return true if point one's tentative distance is shorter
		//Essentially, we want to take priority on the shorter route
		return ( firstPoint._tentative) > ( secondPoint._tentative);
	}
